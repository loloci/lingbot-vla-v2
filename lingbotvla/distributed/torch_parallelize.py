...
# CORRECTNESS-CRITICAL. RAF=False at the 4 sites below is only half the fix; it
# MUST be paired with the step-boundary reshard in train_lingbotvla.py. The two
# escape hatches LINGBOT_KEEP_RESHARD_AFTER_BACKWARD=1 and
# LINGBOT_MANUAL_STEP_RESHARD=0 must be flipped together — never one alone.
        ...
        if parallel_state.dp_mode == "fsdp2":
            ...
            def apply_fsdp_to_decoder_blocks(module: "nn.Module") -> None:
                if module.__class__.__name__ in basic_modules or module.__class__ in ignore_modules_in_mixed_precision:
                    ...
                    # Dual-call: each decoder layer runs forward (and backward) twice
                    # per step. See _KEEP_RESHARD_AFTER_BACKWARD above.
            ...
            if vlm_fsdp:
                ...
                for layer in llm_layers:
                    if layer.__class__.__name__ in FSDP_LAYER_CLASS_NAMES:
                        ...
                        # vlm_fsdp is v2-only; same dual-call loop as expert layers.
                        if not _KEEP_RESHARD_AFTER_BACKWARD:
                            layer.set_reshard_after_backward(enable_full_shard)
