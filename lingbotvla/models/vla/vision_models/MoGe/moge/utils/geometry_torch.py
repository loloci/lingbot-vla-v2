...
def recover_focal_shift_gpu(points: torch.Tensor,
    ...
    if focal_r is None:
        ...
        f_out, s_out = f0, s0
        if solver == "gpu_lm":
            # With a caller-provided focal length, solve the scalar shift
            # update directly so focal remains fixed throughout refinement.
            for _ in range(num_iterations):
                denom = (z + s_out.unsqueeze(-1)).clamp_min(1e-4)
                xy_proj = xy / denom.unsqueeze(-1)
                residual = f0[:, None, None] * xy_proj - uvB
                jacobian = -f0[:, None, None] * xy_proj / denom[:, :, None]
                numerator = (w_bn[:, :, None] * jacobian * residual).sum(dim=(-2, -1))
                denominator = (w_bn[:, :, None] * jacobian.square()).sum(dim=(-2, -1))
                s_out = s_out - numerator / denominator.add(damping)
